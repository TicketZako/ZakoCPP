...
class CliRiskedInterval:
    ...
    def generate():
        ...
        CliUtils.print("", end="\n")
        CliUtils.print(
            "风控等待间隔配置", color="blue", size="large", style="underline"
        )
        CliUtils.print("", end="\n")
        ...
        while True:
            try:
                value = CliUtils.inquire(
                    type="Text",
                    message="请输入风控等待间隔（毫秒）",
                    default=str(configer.setting.riskedInterval),
                )

                if not value:
                    CliUtils.print("风控等待间隔不能为空", color="red")
                    continue

                int_value = int(value)

                if int_value < 0:
                    CliUtils.print("风控等待间隔不能为负数", color="red")
                    continue

                configer.setting.riskedInterval = int_value
                CliUtils.print("", end="\n")
                CliUtils.print(
                    f"✅ 风控等待间隔已设置为: {int_value} 毫秒 ({int_value / 1000:.1f} 秒)",
                    color="green",
                )
                log.info(f"风控等待间隔已设置为: {int_value} 毫秒")
                break
                ...
                log.error(f"配置风控等待间隔失败: {e}")
                break


class CliRefreshInterval:
    """
    刷新余票间隔配置
    """
    ...
    def generate():
        ...
        CliUtils.print("", end="\n")
        CliUtils.print(
            "刷新余票间隔配置", color="blue", size="large", style="underline"
        )
        CliUtils.print("", end="\n")
        ...
        while True:
            try:
                value = CliUtils.inquire(
                    type="Text",
                    message="请输入刷新余票间隔（毫秒）",
                    default=str(configer.setting.refreshInterval),
                )

                if not value:
                    CliUtils.print("刷新余票间隔不能为空", color="red")
                    continue

                int_value = int(value)

                if int_value < 0:
                    CliUtils.print("刷新余票间隔不能为负数", color="red")
                    continue

                configer.setting.refreshInterval = int_value
                CliUtils.print("", end="\n")
                CliUtils.print(
                    f"✅ 刷新余票间隔已设置为: {int_value} 毫秒 ({int_value / 1000:.1f} 秒)",
                    color="green",
                )
                log.info(f"刷新余票间隔已设置为: {int_value} 毫秒")
                break
