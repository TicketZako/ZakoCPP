...
from internal.core.service import BuyerService
...
class CliBuyer:
    ...
    def choose_buyer_step() -> bool:
        ...
        try:
            ...
            selected_buyers = [buyer_map[selected_text] for selected_text in selected]

            configer.buyer.buyer = selected_buyers
            configer.buyer.count = len(selected_buyers)

            CliUtils.print("", end="\n")
            CliUtils.print(
                f"已选择 {len(selected_buyers)} 位购票人",
                color="green",
            )
            for buyer_data in selected_buyers:
